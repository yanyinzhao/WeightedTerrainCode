...
```
./main EP_small.off 0.5 2 0 1
```

In this example, EP_small.off is the terrain data file, epsilon is 0.5, removing value is 2, exact path will not be calculated (and the distance error compared with the exact path and the calculated path will be infinity), and algorithm FixSP will be included (thus, it will run all 18 algorithms).
