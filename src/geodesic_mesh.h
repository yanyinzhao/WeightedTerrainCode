...
namespace geodesic
{
	...
	private:
		template <class Points, class Faces>
		void build_adjacencies(unsigned num_vertices,
							   Points &p,
							   unsigned num_faces,
							   Faces &tri); // build internal structure of the mesh
		bool verify();						// verifies connectivity of the mesh and prints some debug info
		template <class Faces>
		void assign_weight_to_face(unsigned num_faces,
								   Faces &tri); // assign each face with a weight based on the face slope
									...
									Faces &tri)
	{
		...
		for (unsigned i = 0; i < num_faces; ++i) // copy adjacent vertices to polygons/faces
		{
			Face &f = m_faces[i];
			f.id() = i;
			f.weight() = 1;												   // assign a weight to each face
		...
		build_adjacencies(num_vertices, p, num_faces, tri); // build the structure of the mesh
	}
	template <class Points, class Faces>
	inline void Mesh::build_adjacencies(unsigned num_vertices,
										Points &p,
										unsigned num_faces,
										Faces &tri)
	{
		...
		// assign_weight_to_face(num_vertices, p, num_faces, tri);
		assign_weight_to_face(num_faces, tri);
		assert(verify());
	}

	template <class Faces>
	inline void Mesh::assign_weight_to_face(unsigned num_faces,
											Faces &tri) // assign each face with a weight based on the face slope
	{
		for (unsigned i = 0; i < num_faces; ++i) // for each face
		{
			Face &f = m_faces[i];

			// calculate the normal vector of the face
			double face_x_1 = f.adjacent_vertices()[0]->x();
			double face_y_1 = f.adjacent_vertices()[0]->y();
			double face_z_1 = f.adjacent_vertices()[0]->z();
			double face_x_2 = f.adjacent_vertices()[1]->x();
			double face_y_2 = f.adjacent_vertices()[1]->y();
			double face_z_2 = f.adjacent_vertices()[1]->z();
			double face_x_3 = f.adjacent_vertices()[2]->x();
			double face_y_3 = f.adjacent_vertices()[2]->y();
			double face_z_3 = f.adjacent_vertices()[2]->z();
			...
			double face_normal_vector_x = vector_a_y * vector_b_z - vector_a_z * vector_b_y;
			double face_normal_vector_y = vector_a_z * vector_b_x - vector_a_x * vector_b_z;
			double face_normal_vector_z = vector_a_x * vector_b_y - vector_a_y * vector_b_x;

			if (face_normal_vector_z < 0)
			{
				face_normal_vector_x *= -1;
				face_normal_vector_y *= -1;
				face_normal_vector_z *= -1;
			}

			double face_normal_vector_length = sqrt(pow(face_normal_vector_x, 2) + pow(face_normal_vector_y, 2) + pow(face_normal_vector_z, 2));

			double cos_angle = face_normal_vector_z / face_normal_vector_length;
			double angle = acos(cos_angle) * 180.0 / 3.1415926;
